...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Modifier {{ $product->title }} - Coffee Shop</title>
    ...
    <body class="bg-gray-100 min-h-screen flex flex-col justify-between">
        ...
        <main class="container mx-auto px-4 py-8 flex-grow">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-800 text-center">Modifier le produit</h1>
                <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-gray-500 text-sm mt-1">Laissez vide pour conserver l'image actuelle. Formats acceptés: jpeg, png, jpg, gif. Taille max: 2Mo</p>
                        @error('image')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
